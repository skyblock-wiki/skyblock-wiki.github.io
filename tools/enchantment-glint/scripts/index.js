...
/**
 * Creates and loads an HTMLImageElement from a base64 source URL.
 * @param {string} base64src The base64 image source.
 * @returns {Promise<HTMLImageElement>} A promise that resolves with the loaded image element.
 */
function getImage(base64src) {
    return new Promise((resolve) => {
...
Promise.all([getImage(overlayBlocks300), getImage(overlayItems160)]).then((values) => {
...
/**
 * Initializes UI event listeners and restores persisted options after assets are preloaded.
 */
function programReady() {
    ...
    fileForm.addEventListener('submit', async (e) => {
        ...
        const res = (await (await fetch(req)).json())?.query?.pages;
        const imageurl = res ? res?.[Object.keys(res)[0]]?.imageinfo?.[0]?.url : null;
        if (imageurl) {
            const blob = await (await fetch(`https://eejitstools.com/cors-anywhere?url=${imageurl}`)).blob();
...
// Rendering
/**
 * Dispatches rendering with the selected glint format and dimensions.
 * @param {string} filename The output filename (without extension).
 * @param {Blob} baseImageBlob The source image blob to render.
 * @returns {Promise<void>} A promise that resolves when rendering is started.
 */
...
/**
 * Renders the enchanted glint composite to the canvas and updates the downloadable output.
 * @param {string} name The output file base name.
 * @param {HTMLImageElement} image The source texture image.
 * @param {HTMLImageElement} overlayImage The glint overlay image.
 * @param {number} width The render width.
 * @param {number} height The render height.
 */
